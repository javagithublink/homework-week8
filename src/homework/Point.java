...
public class Point {
    ...
    public double distance(){
        double x=0.0, y=0.0;
        double a=Math.sqrt(Math.pow(x - this.x, 2) +
                Math.pow(y - this.y, 2) * 1.0);

        return a;
    }

    public double distance(double x, double y){
        double b=Math.sqrt(Math.pow(x - this.x, 2) +
                Math.pow(y - this.y, 2) * 1.0);
        return b;
    }

    public double distance(double second){
        second = distance(2,2);
        return second;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("distance(second)= "+first.distance(2));
